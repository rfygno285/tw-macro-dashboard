...
// TODO：把下行網址換成 央行 M1B/M2 月報的 xls/xlsx 直連
...
for (const r of raw) {
  ...
  // 2) M2 欄位（可能標示單位，如 NT$ mn / %；這裡直接取數值）
  const m2Key = findM2Key(r);
  let m2 = null;
  if (m2Key) {
    m2 = Number(String(r[m2Key]).replace(/[, ]/g,""));
  }

  if (date && m2 !== null && !Number.isNaN(m2)) {
    out.push({ date, m2 });
  }
}

fs.mkdirSync("docs/data", { recursive: true });
fs.writeFileSync("docs/data/m2.json", JSON.stringify(out, null, 2));
